...
def main():
    ...
    frompasswd=""
    toaddr="" 
    msg_subject="Tutorial 5" 
    msg_body="Tutorial 5 Attendance Report"
    file_path="output/attendance_report_consolidated.xlsx"


    df_reg , df_attendance = read_file("input_registered_students.csv", "input_attendance.csv")
    roll_nm = read_student_file(df_reg)
    date_idx, req_dates =  store_dates(df_attendance)
    create_dataframe(roll_nm, df_attendance, date_idx, req_dates)
    send_mail(fromaddr, frompasswd, toaddr, msg_subject, msg_body, file_path)
...
if __name__ == '__main__':
    ...
    print('Duration of Program Execution: {}'.format(end_time - timestamp))